...
/**
 * this should be the solution for the issue of the screen dimension and have
 * the orientation side ,the text will be from bottom to top on landscape mode
 */
public class ReviewWaveNew extends SurfaceView implements SurfaceHolder.Callback {
    ...
    private void drawWave() {
        ...
        for (int j = 0; j < 10; j++) {
            ...
            for (int i = 0; i < mLeadCount; i++) {
                ...
                // Invert the Y-axis to start from bottom to top
                mCurYs[i] = mViewHeight - (val * 0.001f * gain * perMillmeter * amplitudeScale);
    ...
    public void meanSort() {
        ...
        perLeadHeight = (mViewHeight - paddingTop) / allRow;
        ...
        for (int i = 0; i < mRow; i++) {
            ...
            int whichColumn = (mRow == 0) ? mColumn : i / mRow;
            ...
            // Update the centerline Y to be from the bottom to the top
            mLeadRectFs[i].top = paddingTop + perLeadHeight * (i % mRow);
            mLeadRectFs[i].bottom = mLeadRectFs[i].top + perLeadHeight;

            // Invert the Y-axis (draw from bottom to top)
            mCenterLineYs[i] = mViewHeight - (mLeadRectFs[i].top + mLeadRectFs[i].bottom) / 2;
        }
    }
